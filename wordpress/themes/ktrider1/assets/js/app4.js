...
function gotDevices(deviceInfos) {
  ...
  for (var i = 0; i !== deviceInfos.length; ++i) {
    // console.log("gotDevices foreach!", deviceInfos[i]);
...
function getStream() {
  ...
  // console.log("aaa: ",audioSelect.value);
  // console.log("bbb: ",videoSelect.value);
...
// -- recording
var _correlativo = 0;
var _cada_cuanto = 30 * 2; // 3 segundos?
var tdate = new Date();
var _first_current_timestamp= tdate.getTime();

_first_current_timestamp= tdate.getFullYear().toString(10)//.substring(2)
+ (tdate.getMonth()+1).toString(10).padStart(2,'0')
+ tdate.getDate().toString(10).padStart(2,'0')
+ tdate.getHours().toString(10).padStart(2,'0')
+ tdate.getMinutes().toString(10).padStart(2,'0')  
+ tdate.getSeconds().toString(10).padStart(2,'0');

var _current_timestamp= tdate.getTime();

_time_diff = 0;
console.log("_first_current_timestamp",_first_current_timestamp);
recorderHandleDataAvailable = function(e) {
  // console.log('ondataavailable!',e);
  chunks.push(e.data);
  // console.log("chunks: "+chunks.length);

  if (chunks.length % (_cada_cuanto) == 0) {
    _correlativo++;
    tdate = new Date();
    _time_diff = tdate.getTime() - _current_timestamp;
    console.log("_time_diff",_time_diff,(_time_diff/60/10));
    _current_timestamp= tdate.getTime();
    console.log("_current_timestamp",_current_timestamp);
    console.log("---UPLOAD--- "+chunks.length);

    recorderHandleSave(e);
...
function startRecording() {
  ...
  _correlativo= 0;
  // _first_current_timestamp= new Date().getTime();
  _first_current_timestamp= tdate.getFullYear().toString(10)//.substring(2)
  + (tdate.getMonth()+1).toString(10).padStart(2,'0')
  + tdate.getDate().toString(10).padStart(2,'0')
  + tdate.getHours().toString(10).padStart(2,'0')
  ...
  console.log("recorder started, FPS:", window.stream.getVideoTracks()[0].getSettings().frameRate );
...
function recorderHandleStop(event) {
  ...
  $('#indicador_recording').html("NO. VERIFICANDO VIDEO..");
  ...
  var fileName = ''+_first_current_timestamp+'__'+_correlativo+'__F__.webm';  // or "wav"
  ...
  xhr('/wp-json/booth2/savevid?_wpnonce='+twpnonce, formData, function (fileURL) {
    console.log("!!!! SUBIENDO");
    $('#indicador_recording').html("SUBIENDO VIDEO..", fileName);
  ...
  function xhr(url, data, callback) {
      ...
      request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {
            ...
            console.log("xhr2 onreadystatechange != 4 url:"+url, request.statusText);
          }
      };
...
var chunks_a_guardar= [];

function insertSaveInterval(rchunks) {
  chunks_a_guardar.push(
    setInterval(function(){
      // console.log("chunks_a_guardar!",chunks);
    },4000)
  );
  console.log("insertSaveInterval",chunks_a_guardar);
}

function recorderHandleSave(event) {
  console.log('--recorderHandleSave()!!!: ', event,options,chunks);
  insertSaveInterval(chunks);
  // var blob = new Blob(chunks, { 'type' : 'audio/ogg; codecs=opus' });
  var blob = new Blob(chunks, { 'type' : options.mimeType });
  console.log('--recorderHandleSave, blob', blob);

  $('#indicador_recording2').html( $('#indicador_recording2').html() + " [SUBIENDO VIDEO "+_correlativo+"]");

  chunks= [];


  var fileType = 'video'; // or "audio"
  var fileName = ''+_first_current_timestamp + '__'+_correlativo+'__.webm';  // or "wav"

  var formData = new FormData();
  formData.append(fileType + '-filename', fileName);
  formData.append(fileType + '-blob', blob);
  formData.append('correlativo', _correlativo);

  // xhr('/booth1/save.php?a=savevid', formData, function (fileURL) {
  xhr('/wp-json/booth2/savevid?_wpnonce='+twpnonce, formData, function (fileURL) {
    // console.log("!!!! SUBIENDO");
    // $('#indicador_recording2').html( $('#indicador_recording2').html() + " [SUBIENDO VIDEO "+_correlativo+" ("+fileName+")]");
    // $('#indicador_recording2').html("SUBIENDO VIDEO.. "+ fileName);
  ...
  function xhr(url, data, callback) {
      ...
      request.onreadystatechange = function () {
          if (request.readyState == 4 && request.status == 200) {
              ...
              // $('#indicador_recording2').html( $('#indicador_recording2').html() + " [VIDEO "+_correlativo+" RESP:"+request.responseText+"]");
              // console.log("WHAT2 "+url,data.get('correlativo'),request);
              console.log("xhr onreadystatechange != 4 url:"+url, request.statusText);
          }
      };
      request.open('POST', url);
      request.send(data);
